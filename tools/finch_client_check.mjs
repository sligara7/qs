...
// Waiting for the service to reach a state is NOT one of the things this script checks — it is
// how it gets to the point of checking. Until 2026-09-11 a timeout here returned the last status
// it happened to see, so the NEXT assertion failed on stale data and reported a timing problem
// as a broken response shape. That misdiagnosis cost a CI investigation: the check went red with
// "running_item_uid reported while running — null", which is exactly what a correct service
// reports when the plan has not started yet. A timeout now says it timed out, names what it was
// waiting for and what it last saw, and stops — because every assertion after a missed state
// transition is noise.
class WaitTimeout extends Error {}
async function waitFor(what, pred, timeoutMs = 60000, every = 150) {
  ...
  const seen = s
    ? `re_state=${s.re_state} manager_state=${s.manager_state} running_item_uid=${s.running_item_uid} items_in_queue=${s.items_in_queue} items_in_history=${s.items_in_history}`
    : 'no status was read';
  check(`waiting for ${what}`, false, `TIMED OUT after ${timeoutMs} ms; last status: ${seen}`);
  throw new WaitTimeout(what);
}
...
async function run() {
  ...
  s = await waitFor('the count plan to finish', (s) => s.items_in_history >= 1 && s.running_item_uid === null);
  ...
  // 30 s rather than 15: on a loaded two-core CI runner the sequencer has been seen to take
  // longer than 15 s to pick the item up and start it. This bound is how long the service may
  // take to START a plan, not how long the plan runs.
  s = await waitFor('the executed scan to start running', (s) => s.re_state === 'running', 30000);
  ...
  s = await waitFor('the scan to pause at its next checkpoint', (s) => s.re_state === 'paused', 20000);
  ...
  s = await waitFor('the resumed scan to run again', (s) => s.re_state === 'running', 15000);
  check('abortRE', (await post('/re/abort')).success);
  s = await waitFor('the aborted scan to reach the history', (s) => s.re_state === 'idle' && s.running_item_uid === null && s.items_in_history === 2, 30000);
...
try {
  ...
  if (err instanceof WaitTimeout) {
    // The timeout is already reported above as a FAIL line naming what it waited for. Stopping
    // here keeps the run from producing a cascade of assertions against a service that never
    // reached the state they assume.
    console.log('\nStopped early: the service did not reach an expected state in time.');
  } else {
    check('the check ran to completion', false, `${err}`);
  }
}
